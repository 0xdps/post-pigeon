...
export async function getNextFromQueue() {
	const db = getDb();
	const items = await db.find("queued", undefined, { orderBy: "created_at", order: "ASC", limit: 1 });
	return items[0] ?? null;
}
...
export async function removeQueueEntriesForItem(localId) {
	const db = getDb();
	await db.delete("queued", { local_id: String(localId) });
}

export async function getQueuedLocalIds() {
	const db = getDb();
	const items = await db.find("queued", undefined, { columns: ["local_id"] });
	return new Set(items.map((r) => String(r.local_id)));
}

/**
 * Returns a Set of all local_ids that should be skipped when picking the next
 * bank item: anything currently queued, any pending scheduled post, and any
 * history entry that has not been soft-deleted.
 * Uses a single UNION query instead of three round-trips.
 */
export async function getBlockedLocalIds() {
	const db = getDb();
	const result = await db.exec(`
		SELECT local_id FROM queued
		UNION
		SELECT local_id FROM scheduled WHERE status = 'pending'
		UNION
		SELECT local_id FROM history WHERE deleted_at IS NULL
	`);
	return new Set(result.rows.map((r) => String(r.local_id)));
}

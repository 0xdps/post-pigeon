...
/**
 * Load all bank posts from DB, sorted by legacy_id numerically.
 * @returns {Promise<Array<{id: string, postId: string, title: string}>>}
 */
export async function loadBankFromDb() {
	const db = getDb();
	const result = await db.exec(
		"SELECT * FROM posts WHERE json_extract(metadata, '$.source') = 'bank-sync' ORDER BY CAST(json_extract(metadata, '$.legacy_id') AS INTEGER) ASC"
	);
	const rows = Array.isArray(result?.rows) ? result.rows : [];
	return rows.map((p) => {
		const meta = JSON.parse(p.metadata || "{}");
		return { id: meta.legacy_id, postId: p.id, title: p.title, text_file_id: meta.text_file_id ?? null };
	});
}

/**
 * Get a single bank post from DB by legacy_id.
 * @param {string} id - legacy_id (e.g. "001" or "1")
 * @returns {Promise<{id: string, postId: string, title: string} | null>}
 */
export async function getBankItemFromDb(id) {
	const db = getDb();
	// Try direct postId lookup first
	let post = await db.findById("posts", `bank-${id}`);
	if (!post) {
		// Fallback: search by legacy_id in metadata (handles unpadded ids)
		const result = await db.exec(
			"SELECT * FROM posts WHERE json_extract(metadata, '$.source') = 'bank-sync' AND json_extract(metadata, '$.legacy_id') = ? LIMIT 1",
			[String(id)]
		);
	...
	return { id: meta.legacy_id, postId: post.id, title: post.title, text_file_id: meta.text_file_id ?? null };
}
